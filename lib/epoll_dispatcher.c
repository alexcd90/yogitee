...
int epoll_add(struct event_loop *eventLoop, struct channel *channel1) {
    epoll_dispatcher_data *pollDispatchData = (epoll_dispatcher_data *) eventLoop->event_dispatcher_data;
    ...
    if (epoll_ctl(pollDispatchData->efd, EPOLL_CTL_ADD, fd, &event) == -1) {
